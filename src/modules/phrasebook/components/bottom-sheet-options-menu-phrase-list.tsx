...
export const BottomSheetOptionsMenuPhraseList: FC<Props> = ({ show, onClose }) => {
  ...
  const renderBackdropBottomSheet = useCallback(
    (backdropProps: BottomSheetBackdropProps) => (
      <BottomSheetBackdrop {...backdropProps} disappearsOnIndex={-1} appearsOnIndex={0} opacity={0.75} />
    ),
    [],
  );
  ...
  return (
    <BottomSheetModal
      ...
      bottomInset={createSpacing(screenUtils.isSmallScreen ? 4 : 6)}
...
const styles = StyleSheet.create({
  bottomSheet_root: {
    flex: 1,
    marginHorizontal: createSpacing(screenUtils.isSmallScreen ? 4 : 5),
  },
